...
- **`agent-browser`'s snapshot/ref model** → load the `kernel-agent-browser` skill and follow its CDP attachment workflow, reusing the browser you already created in Step 1 rather than minting a second one. That skill owns the current attachment syntax and provider/profile constraints; use its "Creating Site-Specific Browser Automation Skills" reference for the login and task-sequence command shapes.
